...
element_shell_re = regex.compile(r'^\s*(?P<sym>\w+)\s+(?P<am>[spdfghikSPDFGHIK])\s+(?:\s*?(?P<alias>\w+)\s*?)\s*:\s*(?P<nprim>\d+)\s+(?P<ncontr>\d+)\s+(?:\s*(?P<range>\d+.\d+)\s*)\s*$')
...
def _parse_electron_lines(basis_lines, bs_data):
    ...
    while element_shell_re.match(basis_lines[iline]):
        ...
        cranges = [ r.split[0] for r in cranges ]
        ...
        while nread < nreadin:
            iline += 1
            entry = helpers.parse_line_regex_dict(entry_re, basis_lines[iline], 'exponent / contraction')
            rawdata = rawdata + entry
            nreadin += len(entry)
