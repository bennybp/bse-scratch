...
element_shell_re = regex.compile(
    r'^\s*(?P<sym>\w+)\s+(?P<am>[spdfghikSPDFGHIK])\s*(?:\s*(?P<alias>{})\s*)+\s*:\s*(?P<nprim>\d+)\s*(?P<ncontr>\d+)\s*(?:\s*(?P<range>\d+.\d+)\s*)+\s*$'.format(basis_set_name_re_str)
)
...
ecp_re = regex.compile(
    r'\s*(?P<sym>\w+)\s+ECP\s+(?:\s*(?P<alias>{})\s*)\s*:\s*(?P<ncore>\d+)\s+(?P<lmax>\d+)\s+(?P<lmaxso>\d+)\s+(?P<ndata>\d+)\s*$'.format(basis_set_name_re_str)
)
